...
**本轮新增底层：** 内容暂存、用户确认保存、不可变版本及素材备份接口；通用文本、中英文和 LinkedIn 普通动态固定规则。界面仍为原五步模板，完整看稿改稿和生产级 Agent 执行尚未接入，当前“复制任务”用于交接，不会启动 Agent。

**后续范围：** 创作 → 发布 → 反馈（留空）；创作内部生成初稿、自动审核一次、修订最终稿。中英文长文、kit/copy、DOCX 及其余平台适配仍保留在完整需求中。本轮不宣布 MVP 打通、不自动进入下一轮。见 [第一轮实施范围](https://github.com/agtai/launch-loop/blob/main/docs/第一轮实施范围.md) 和 [最新验证](https://github.com/agtai/launch-loop/blob/main/docs/验证记录.md)。
...
- **保存位置：** `data/workbench.sqlite` 存放模块、任务及已确认内容版本和素材。未经确认的内容与素材只在 `data/tmp/content/`；不自动清理。`data/` 不进入 Git；克隆仓库不会带走维护者的运营数据。
- **换电脑或交给下一位执行者：** 在页面点击“备份数据”，得到 JSON；对方启动自己的工作台后选择“恢复备份”，核对预览再确认。
- **备份范围：** JSON 包含模块、任务及已确认内容的所有版本和选定素材字节，不包含 tmp、应用源码、未导入的 Word 文件或外部成果链接指向的文件。临时稿需随完整 `data/` 单独交接。
- **自动快照：** 每天首次修改前保留一份快照，最近 30 份；每次恢复前另存快照，最近 20 份，均位于 `data/backups/`。
- **恢复要求：** 协议仍为版本 1、最大 16 MiB。旧备份恢复模块与任务并保留现有正式内容；含 `content` 扩展的新备份还恢复正式内容集合及其素材。校验失败不替换数据。成功恢复后旧 tmp 保留可读，须显式派生新暂存再编辑；详见 [内容存储接口](https://github.com/agtai/launch-loop/blob/main/docs/内容存储接口.md)。旧程序不能恢复新增 `content` 扩展。
...
> 请先读取 AGENTS.md、docs/SESSION_HANDOFF.md 和 docs/第一轮实施范围.md，核对 Git、源码和验证记录。第一轮只交付基线、接入预检、存储底层与规则；未经新授权不自动启动第二轮。后续优先复用已验证入口和固定规则，真实生成内容先 tmp、用户确认后才正式保存；不要把会话工具可用写成工作台接入完成。

...
test('HTTP publishing routes preserve CSRF, durable dedupe, backup merge and callback boundaries', async t => {
  ...
  const request = (route, body, method = body === undefined ? 'GET' : 'POST', headers = {}) => fetch(app.url + route, { method, redirect: 'manual', headers: { ...(body === undefined ? {} : { 'Content-Type': 'application/json' }), ...headers }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
  ...
  assert.ok(failureHtml.includes(`href="${app.url}/#publishing"`));
  assert.doesNotMatch(failureHtml, /RAW_\w+_SENTINEL|evil\.example|TOKEN_SENTINEL|SECRET_SENTINEL|<script/i);
  assert.equal((await request('/api/linkedin/connection/start', { returnOrigin: 'https://evil.example.test' })).status, 400);
  assert.equal((await request('/api/linkedin/connection/start', { locale: 'invalid' })).status, 400);
  const cancelled = new URL((await ok('/api/linkedin/connection/start', { locale: 'en' })).authorizationUrl);
  const cancellation = await request(`/api/linkedin/callback?state=${cancelled.searchParams.get('state')}&error=access_denied&error_description=RAW_ERROR_SENTINEL`);
  assert.equal(cancellation.status, 400);
  const cancelledHtml = await cancellation.text();
  assert.ok(cancelledHtml.includes('<html lang="en">')); assert.doesNotMatch(cancelledHtml, /RAW_ERROR_SENTINEL/);
  assert.equal((await ok('/api/linkedin/connection')).connection.status, 'disconnected');
  const localOrigin = `http://localhost:${app.port}`;
  const localStart = await fetch(`${localOrigin}/api/linkedin/connection/start`, { method: 'POST', redirect: 'manual', headers: { 'Content-Type': 'application/json', Origin: localOrigin }, body: '{}' });
  const localStartBody = await localStart.json(); assert.equal(localStart.status, 200, JSON.stringify(localStartBody));
  const localAuth = new URL(localStartBody.authorizationUrl);
  const localCallback = await request(`/api/linkedin/callback?state=${localAuth.searchParams.get('state')}&code=SYNTHETIC&returnUrl=https://evil.example.test`);
  assert.equal(localCallback.status, 303); assert.equal(localCallback.headers.get('location'), `${localOrigin}/#publishing?linkedin=connected`);
  const auth = new URL((await ok('/api/linkedin/connection/start', { locale: 'en' })).authorizationUrl);
  const callbackPath = `/api/linkedin/callback?state=${auth.searchParams.get('state')}&code=SYNTHETIC`;
  const callback = await request(callbackPath, undefined, 'GET', { Origin: 'https://www.linkedin.com', 'Sec-Fetch-Site': 'cross-site' });
  assert.equal(callback.status, 303); assert.equal(callback.headers.get('location'), `${app.url}/#publishing?linkedin=connected`);
  assert.equal(callback.headers.get('cache-control'), 'no-store'); assert.equal(callback.headers.get('referrer-policy'), 'no-referrer');
  assert.equal(await callback.text(), ''); assert.equal((await request(callbackPath)).status, 401);
  assert.equal((await ok('/api/linkedin/connection')).connection.canPublish, true); assert.equal(postCount, 0);

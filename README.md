...
**外部能力有条件：** 文本执行器通过本机已登录的Codex CLI调用真实模型，实际验收结果见[验证记录](docs/验证记录.md)。自动配图尚未连接，可选用上传图片。LinkedIn普通动态发布适配已实现，仍需应用权限、HTTPS回调和用户OAuth授权，尚未真实发帖；确认保存不会自动发布。本地资料和任务功能无需Codex账号。

本轮72项本地自动测试通过；两次从工作台触发的真实生成均遇到当前网络连接超时，未获得正文，真实双语生成／审核／修订和Agent改稿仍待网络恢复后验收。已有试稿的界面与存储验证不替代真实生成结果。

**本阶段范围：** 一个产品、LinkedIn普通feed、中英文独立稿件与配图路径。中英文长文章、kit/copy、DOCX、其他四个平台和视频仍保留后续需求。当前不宣称完整MVP已打通。见[第二阶段接口与分工](docs/第二阶段接口与分工.md)。
...
发布页显示具体缺失配置并使用正式LinkedIn OAuth。操作者须在本机安全配置应用及HTTPS回调；不要在聊天中粘贴secret。预览绑定账号、明确版本和图片，换账号或恢复后需重新确认；未知发布结果禁止自动重试。配置、权限、接口和恢复规则见[执行与发布接口](docs/第二阶段执行与发布接口.md)。

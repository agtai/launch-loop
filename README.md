...
**当前范围：** 一个产品、LinkedIn普通feed、中英文独立稿件与配图路径。中英文长文章、kit/copy、DOCX、其他四个平台和视频仍保留后续需求。当前不宣称完整MVP已打通。实施边界见[本轮范围与验收](docs/MVP本轮接口与验收.md)。
...
本轮用户已批准官方图片缓存例外。后台配图启动命令为 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Start-Workbench.ps1 -EnableCodexImages`，需要现有系统代理时加 `-UseSystemProxy`；已运行服务先使用同目录停止脚本。图片先由正式工具写官方缓存，再收进项目tmp并实际看图检查。改稿造成正文hash变化时需更新或取消选用图片；失败保留文本，不自动重试。

发布页显示具体缺失配置并使用正式LinkedIn OAuth。先按[最小配置步骤](docs/LinkedIn最小配置.md)准备应用及仅开放回调路径的HTTPS资源；不要在聊天中粘贴secret。授权成功返回工作台并读取实际连接状态。预览绑定账号、明确版本和图片，换账号或恢复后需重新确认；未知发布结果禁止自动重试。接口和兼容规则见[MVP执行与素材接口](docs/MVP执行与素材接口.md)。

...
**后续范围：** 创作 → 发布 → 反馈（留空）；创作内部生成初稿、自动审核一次、修订最终稿。中英文长文、kit/copy、DOCX 及其余平台适配仍保留在完整需求中。本轮不宣布 MVP 打通、不自动进入下一轮。见 [第一轮实施范围](docs/第一轮实施范围.md) 和 [最新验证](docs/验证记录.md)。
...
- **恢复要求：** 协议仍为版本 1、最大 16 MiB。旧备份恢复模块与任务并保留现有正式内容；含 `content` 扩展的新备份还恢复正式内容集合及其素材。校验失败不替换数据。成功恢复后旧 tmp 保留可读，须显式派生新暂存再编辑；详见 [内容存储接口](docs/内容存储接口.md)。旧程序不能恢复新增 `content` 扩展。

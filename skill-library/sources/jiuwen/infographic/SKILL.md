...
- **An image-model key — one of these is enough:**
  - `GEMINI_API_KEY` (from https://aistudio.google.com/apikey), exported or placed in `.env`; or
  - **nothing extra when running inside jiuwen**: if `GEMINI_API_KEY` is absent, `generate-diagram.ts` automatically uses the OpenRouter key already configured in jiuwen (or `OPENROUTER_API_KEY`) to call the same model, `google/gemini-3.1-flash-image-preview`, with all three reference images. Do NOT stop or ask the user for a Gemini key in this case — just run the script.
  - Each diagram costs roughly $0.07 through OpenRouter.
- Node.js with `npx` available. The generator runs via `tsx` and the screenshot step uses Playwright.
- Run `npm install` once in the skill directory if `node_modules/` is missing (it also fetches a headless Chromium for Playwright). Skip it when `node_modules/` already exists.

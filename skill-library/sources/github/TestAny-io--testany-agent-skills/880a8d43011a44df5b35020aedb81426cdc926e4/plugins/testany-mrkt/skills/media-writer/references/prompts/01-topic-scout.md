...
2. 检查要素：
   ...
   - [ ] 有没有具体的写作方向？
   - [ ] 明确了投放平台吗？
3. 如果 brief 完整，返回已满足输入；协调者按模式继续或在 Brief 检查点停下
...
#### Step 5: 仅在主题仍需作者决定时请求选择

已有明确主题，或用户已授权在给定范围内选题时，记录所用主题并继续；不重复索要选择。主题仍未确定且会改变作者立场或范围时，只暂停依赖该决定的工作。

**需要澄清时，使用可用提问工具或普通文本。以下仅为结构化提问示例：**
...
交接：Brief 完成；协调者按模式决定是否进入素材收集，检查点任务在此结束。
```

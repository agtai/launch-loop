...
import type {ContentBrief, ContentDocument, ContentDraft, ContentItem, ContentSource, ContentSummary, ContentTmp, ContentUploadInput, ContentVersion, GenerationCapabilities, GenerationJob, GenerationOptions, GenerationRequest, ModificationRequest} from './content-types';
import XDocuments from './XDocuments';
import XImageTasks from './XImageTasks';
...
const isServiceNotice = (value: string) => value === '自动配图后端未连接；没有生成图片，本稿仍需处理配图。' || value === '局部修改后未再次自动审核，请检查与全文的一致性。' || value === '正文已修改，相关 X 配图需要重新核对或更新。' || /^LinkedIn 动态正文超过 3000 限制（含标题：\d+ 个 Unicode 字符，\d+ 个 UTF-16 单位），需缩短后再发布。$/.test(value);
...
export default function ContentWorkspace({onDirtyChange, active = true, dataEpoch = 0}: {onDirtyChange?: (dirty: boolean) => void; active?: boolean; dataEpoch?: number}) {
  ...
  async function persist(): Promise<ContentTmp | null> {
    ...
    const promise = contentApi<{item: ContentTmp}>(`/api/content/tmp/${current.id}`, 'PUT', {revision: current.revision, content: body, temporary: current.temporary})
      .then(({item}) => {
        ...
        tmpRef.current = item; setTmp(item);
        if (editEpoch.current === epoch) {draftRef.current = structuredClone(item.content); setDraft(structuredClone(item.content)); dirtyRef.current = false; setDirty(false); setSaveState('已暂存 · 待确认保存');}
  ...
  function editXDocuments(documents: ContentDocument[]) {
    if (!draft || !editable) return;
    const next = {...draft, documents};
    editEpoch.current++; dirtyRef.current = true; draftRef.current = next; setDraft(next); setDirty(true); setSelection(null); setSaveState('有修改等待暂存');
  }
  async function xImageAction(action: 'prepare' | 'bind' | 'verify', documentId?: string, assetId?: string, check?: {altText: string; generated: boolean}) {
    await guarded(async () => {
      const current = await persist(); if (!current) return;
      const {item} = await contentApi<{item: ContentTmp}>(`/api/x-images/tmp/${current.id}/${action}`, 'POST', {revision: current.revision, ...(documentId ? {documentId} : {}), ...(assetId ? {assetId} : {}), ...(action === 'verify' ? {confirmed: true, ...check} : {})});
      installTmp(item);
    });
  }
  async function uploadXImage(file: File, documentId: string) {
    await guarded(async () => {
      const current = await persist(); if (!current) return;
      const {id: _id, ...upload} = await uploadInput(file);
      const uploaded = await contentApi<{item: ContentTmp; asset: {id: string}}>(`/api/content/tmp/${current.id}/assets`, 'POST', {revision: current.revision, ...upload});
      installTmp(uploaded.item);
      const {item} = await contentApi<{item: ContentTmp}>(`/api/x-images/tmp/${current.id}/bind`, 'POST', {revision: uploaded.item.revision, documentId, assetId: uploaded.asset.id});
      installTmp(item);
    });
  }
  ...
  const imageOutdated = shown?.platform === 'linkedin' && assets.some(asset => shown?.assetIds.includes(asset.id) && asset.imageBinding && asset.imageBinding.documentsHash !== shownHash);
  ...
  const pendingLanguageImpact = jobs.find(job => job.kind === 'generation' && (job.languageImpact?.sourcePlatform ?? 'linkedin') === shown?.platform && job.languageImpact?.affectedLanguages.includes(shown?.language as 'zh'|'en') && job.variants.some(variant => variant.tmpId === tmp?.id))?.languageImpact;
  ...
  useEffect(() => {
    if (!tmp || tmp.content.platform === 'x' || dirtyRef.current || conflictRef.current || !activeJob || running(activeJob)) return;
    let live = true;
  ...
  const unsupported = brief.platforms.some(p => !['linkedin', 'x'].includes(p)) || brief.formats.includes('long_article') || brief.platforms.includes('linkedin') && brief.formats.includes('thread');
        ...
        <form noValidate onSubmit={event => {event.preventDefault(); void generate();}}>
          <fieldset disabled={busy} className='cw-form-fields'>
            ...
            <details className='cw-options' open><summary>{t("可选配置")}<ChevronDown size={16}/></summary><p className='cw-help'>{t("本次试运行预设：LinkedIn、中文和英文、普通动态。可以修改；这不是全局默认值。")}</p>
              ...
              <Checks label={t("内容格式")} options={[['short_post', '普通动态'], ['long_article', '长文章 / Pulse（待适配）'], ['thread', '串帖（X）']]} selected={brief.formats} onChange={formats => setBrief({...brief, formats})}/>
              <p className='cw-help'>{t('仅平台 × 语言拆稿：当前 {count} 份。其他选项合并到每份稿件。', {count: brief.platforms.length * brief.languages.length})}</p>
              {unsupported && <p className='cw-warning'>{t("当前支持 LinkedIn 普通动态及 X 普通帖子／串帖。其他平台、长文章或 LinkedIn 串帖尚未适配。")}</p>}
            ...
            <p className='cw-help'>{t("上传支持 PNG / JPEG；LinkedIn 单图、X 普通帖子单图及串帖首图。未上传时准备自动配图。")}{brief.platforms.includes('linkedin') && (capabilities?.image.message ? t(capabilities.image.message) : t("正在检查配图连接…"))}{brief.platforms.includes('x') && t('配图状态在稿件中单独显示')} {t("每个文件最多 4 MiB。")}</p>
            {brief.platforms.includes('linkedin') && capabilities?.image.canRefresh && <button type='button' className='button small-btn' disabled={busy || capabilities.image.checking} onClick={() => void guarded(async () => {setCapabilities(await contentApi<GenerationCapabilities>('/api/generation/capabilities/image/refresh', 'POST', {}));})}><RefreshCw size={15}/>{t(capabilities.image.checking ? '正在检查配图连接…' : '重新检查配图入口')}</button>}

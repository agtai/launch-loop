  ...
  return <div className='cw'>
    ...
    <div className='cw-columns'>
      ...
      <div className='cw-writing-column'>
        {!!jobs.length && <section className='panel cw-jobs'><div className='cw-section-heading'><h2>{t("本机创作任务")}</h2><small>{t("实际执行状态")}</small></div>{jobs.slice(0, 8).map(job => <div className='cw-job' key={job.id}><div className='cw-job-header'><strong>{job.kind === 'modification' ? t("局部改稿") : t("生成与审核")}</strong><span className='tag'>{t(stageNames[job.status === 'running' ? job.stage : job.status] || job.stage)}</span><small>{date(job.createdAt)}</small>{running(job) && <button className='button small-btn' disabled={busy} onClick={() => void cancelJob(job)}>{t("取消")}</button>}</div>{job.error && <p className='cw-warning'>{t(job.error)}</p>}<div className='cw-variants'>{job.variants.map(variant => <button className={'cw-variant ' + (tmp?.id === variant.tmpId ? 'selected' : '')} key={variant.platform + variant.language} disabled={!variant.tmpId || running(job) && variant.status !== 'completed' || busy} onClick={() => variant.tmpId && void openJobVariant(job, variant.tmpId)}><strong>{t(platformNames[variant.platform])} {t("·")}{t(languageNames[variant.language])}</strong><span>{t(stageNames[variant.stage] || variant.stage)}</span>{variant.error && <small>{t(variant.error)}</small>}{variant.assetStatus === 'unconnected' && <small>{t("自动配图未连接")}</small>}</button>)}</div>{!running(job) && ['failed', 'cancelled', 'interrupted'].includes(job.status) && job.kind === 'generation' && <button className='text-btn' disabled={busy} onClick={() => {requestRef.current = null; void generate();}}>{t("按当前左侧资料重新生成")}</button>}</div>)}</section>}
        <section className='panel cw-editor' aria-label={t("正文编辑器")}>
          ...
          {!shown ? <div className='cw-empty'><FileText size={42}/><h3>{t("把想说的事，写清楚。")}</h3><p>{t("填写左侧三项资料，生成后可切换中英文、直接编辑段落，也可以选中文字提交修改意见。")}</p><span>{t("初稿 · 一次自动审核 · 修订稿")}</span></div> : <>
            ...
            {!!unresolvedFindings.length && <section className='cw-review'><h3>{t("尚未解决的审核问题")}</h3><ul>{unresolvedFindings.map((finding, index) => <li key={index}>{isServiceNotice(finding) ? t(finding) : finding}</li>)}</ul></section>}

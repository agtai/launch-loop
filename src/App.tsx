...
export default function App() {
    ...
    const [publishingPlatform] = useState<'x' | 'linkedin'>(window.location.hash === '#publishing?x=connected' ? 'x' : 'linkedin');
    ...
    useEffect(() => {
        if (!['#publishing', '#publishing?linkedin=connected', '#publishing?x=connected'].includes(window.location.hash)) return;
 ...
 {publishingOpened && <div hidden={view !== 'publishing'}><PublishingWorkspace active={view === 'publishing'} dataEpoch={contentEpoch} initialPlatform={publishingPlatform}/></div>}

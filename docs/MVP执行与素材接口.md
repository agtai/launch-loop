...
2026-09-26：后续通用任务控制的行为要求见[任务交互与恢复规范](任务交互与恢复规范.md)，当前本文的端点、阶段预算及两个一次续接限制仍有效。新增规范未实现通用暂停／继续，也未引入自动重试；实际接口替代及旧任务兼容须由 UX-03／CORE-01 明确并验证。X 按其[独立接口](x/implementation.md)处理，不将下文 LinkedIn 范围误读为 main 不支持 X。
